...
public class ImageConvolution
{
  ...
  public static void ConvoluteImage(String imgIn, String imgOut, int kWidth, int kHeight, int kDivisor, int[] kValues) throws IOException
  {
    int kernelWidth = kWidth;
    int kernelHeight = kHeight;
    int kernelDivisor = kDivisor;
    ...
    for (int i = 0; i < kernelHeight; i++)
    {
     //System.out.print("[");
      ...
      {
        kernel.set(j, i, kValues[y++]);
        //System.out.print(" " + kernel.get(j, i) + " ");
      }
      //System.out.println("]");
    }
 
    ArrayData[] dataArrays = getArrayDatasFromImage(imgIn);
    for (int i = 0; i < dataArrays.length; i++)
      dataArrays[i] = convolute(dataArrays[i], kernel, kernelDivisor);
    writeOutputImage(imgOut, dataArrays);
    return;
  }
}
